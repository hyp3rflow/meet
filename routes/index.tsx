...
export async function handler(
  ...
  const maybeAccessToken = getCookies(req.headers)["meet_token"];
...
export default function Main({ url, data }: PageProps<{ rooms: RoomView[] }>) {
  return (
    <>
      <div
        class={tw`absolute top-0 left-0 w-full min-h-screen -z-10 bg-white`}
      />
        ...
        <div>
          ...
          {data
            ? (
              <ul
                role="list"
                class={tw`max-h-[21.375rem] overflow-y-scroll space-y-2`}
              >
                <li>
                  <a
                    href="/new"
                ...
                {data.rooms.map((room) => {
                  return (
                    <li key={room.roomId}>
                      ...
                      >
                        <p class={tw`justify-self-start ml-8`}>
                          {Object.values(room.participants).filter(Boolean)
                            .length} participants
                        </p>
                        <p
                          class={tw
